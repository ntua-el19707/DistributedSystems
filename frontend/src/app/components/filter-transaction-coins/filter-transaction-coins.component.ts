...
export class FilterTransactionCoinsComponent implements OnDestroy {
  ...
  @Input() filter$ :BehaviorSubject<filterTranctionCoinSubject> = new  BehaviorSubject({})
  constructor(private filterTransactionCoinToaBehavirSubjectService:FilterTransactionCoinToaBehavirSubjectService){
    this.#Subscription =  this.DateAndTimeForm$.subscribe(r=>{
      this.filterForm.controls.DateAndTimeForm = r  
      this.filterTransactionCoinToaBehavirSubjectService.next( this.filter$,  this.filterForm)
    })
  }
  ngOnDestroy(): void {
...
Start: new FormControl<string | null>(null, { validators:[Validators.pattern(timePattern)   ,TimeValidator]}),
 ...
 new FormGroup<TransactionCoinFilterForm>({
  ...
  coinsMax:new FormControl<string | null>(null,[Validators.pattern(/^[0-9]+(\.[0-9]+)?$/)]),
  coinsMin:new FormControl<string | null>(null,[Validators.pattern(/^[0-9]+(\.[0-9]+)?$/)]),
