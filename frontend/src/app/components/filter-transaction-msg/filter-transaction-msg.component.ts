...
@Component({
  ...
  standalone: true,
  imports: [...material, ...forms, DateAndTimePickerComponent ,AsyncPipe ,CommonModule],
...
export class FilterTransactionMsgComponent {
  ...
  constructor(
    private filterTransactionMsgToaBehavirSubjectService: FilterTransactionMsgToaBehavirSubjectService,
    ...
    this.#Subscription = this.DateAndTimeForm$.subscribe((r) => {
      ...
      this.filterTransactionMsgToaBehavirSubjectService.next(
        this.filter$,
        this.filterForm
      );
    });
